...
struct ARTest: View {
    var body: some View {
        ARTestControllerWrapper()
    }
}

/**
 UIKitをSwiftUIで使用できるように変換
 */
struct ARTestControllerWrapper: UIViewControllerRepresentable {

    func makeUIViewController(context: Context) -> ARTestController {
        return ARTestController()
    }


    func updateUIViewController(_ uiViewController: ARTestController, context: Context) {
        uiViewController.arView = "ARView"
    }
}



class ARTestController: UIViewController, ARSessionDelegate {
    ...
    @IBAction func tappedExportButton(_ sender: UIButton) {
        guard (arView.session.currentFrame?.camera) != nil else {return}
